...
class RunConfig:
    ...
    num_lang: Optional[int] = field(
        default=1, metadata={"help": "num of langs for multilingual training"}
    )

@hydra.main(config_name=None, version_base=None)
def main_hydra(cfg: DictConfig):
    run_config = RunConfig()
    # print(f'seeme seeme seeme seeme ---------------------{OmegaConf.to_yaml(run_config.model_config)}--------------------------------------')
    ...
    if kwargs.get("debug", False):
        import pdb;
        pdb.set_trace()
    # print(f"---------------------------------------{OmegaConf.to_yaml(cfg.model_config)}------------------------")
    # print(f"---------------------------------------{kwargs}------------------------")
