...
# from slam_llm.utils.dataset_utils import DynamicBatchSampler
...
def main(kwargs: DictConfig):
    ...
    model_factory_out = model_factory(train_config, model_config, **kwargs)
    indic_whisper_wrapper = None
    if len(model_factory_out) > 2:
        model, tokenizer, indic_whisper_wrapper = model_factory_out
    else:
        model, tokenizer = model_factory_out


    # Load model checkpoint if specified (only model weights)
    if kwargs.get("ckpt_path") and os.path.exists(kwargs.ckpt_path):
        logger.info(f"✅ Loading model checkpoint from {kwargs.ckpt_path}")
        try:
            state_dict = torch.load(kwargs.ckpt_path, map_location='cpu')
            model.load_state_dict(state_dict, strict=False)
        except Exception as e:
            logger.error(f"❌ Failed to load checkpoint: {e}")
    else:
        logger.warning("⚠️ No valid checkpoint found or ckpt_path not provided. Training will start from scratch.")
    ...
    dataset_train = get_preprocessed_dataset(
        ...
        whisper_model=indic_whisper_wrapper
        )
    if not (train_config.enable_fsdp or train_config.enable_ddp) or rank == 0:
