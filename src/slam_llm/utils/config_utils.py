...
from slam_llm.utils.dataset_utils import DistributedDynamicBatchSampler
...
def get_dataloader_kwargs(train_config, dataset, tokenizer, mode):
        ...
        elif train_config.batching_strategy == "frame_level":
            # --- Use Your New Sampler ---
            kwargs["batch_sampler"] = DistributedDynamicBatchSampler(
                    dataset,
                    max_frames_per_batch=batch_size,  
                    num_replicas=None,  # <--- CHANGE THIS: Do not call dist.get_world_size()
                    rank=None,          # <--- CHANGE THIS: Do not call dist.get_rank()
                    # shuffle=(mode == "train"),
                    shuffle=False,
                    seed=42
                )
                            
            # --- CRITICAL: Remove conflicting standard args ---
            # The sampler now handles size, shuffling, and drop_last internally
