    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                eval_ppl, eval_epoch_loss, *rest, lang_mask = evaluation(model, train_config, eval_dataloader, local_rank, tokenizer)
                quit()
    ...
    with MemoryTrace() as memtrace:
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                ...
                acc = rest[0] if rest else -1
                loss = outputs.loss
                all_lang_masks.append(batch['lang_mask'].cpu().numpy())
                all_utterance_probs.append(rest[2]['utterance_probs'].cpu().numpy())
        ...
        final_masks_np = np.vstack(all_lang_masks)
        final_probs_np = np.vstack(all_utterance_probs)
                # Convert numpy arrays back to PyTorch tensors for saving
        final_masks_tensor = torch.from_numpy(final_masks_np)
        final_probs_tensor = torch.from_numpy(final_probs_np)

        # Create a dictionary to hold the tensors
        results_to_save = {
            'final_masks': final_masks_tensor,
            'final_probs': final_probs_tensor
        }

        # Save the dictionary to a file
        output_filename = '/raid/isha/SpeechLLM/SLAM-LLM/outputs/analysis/smear/probs_and_masks_kathbath_ml.pt'
        print(f" Saving in file {output_filename}........")
